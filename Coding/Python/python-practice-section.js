/* python-practice-section.js — §V "Practice & Mistake Bank" for the Python index.
 * Native plain-JS port of the python-practice-lab.jsx prototype: a graded practice
 * loop powered by real in-browser CPython (Pyodide), an attempts log, a localStorage
 * "mistake bank", and a Mistake Drill.
 *
 * v1 scope: localStorage-only (NO Supabase reads/writes), no sign-in gate. Lazy-loads
 * its OWN Pyodide instance on the first "Run & Check" — the page's existing 100 Days
 * runner (__RUNNER) is left untouched. Renders into #pmb-root; the section heading +
 * engine pill live in index.html.
 */
(function () {
  "use strict";
  if (window.__pmbLoaded) return; window.__pmbLoaded = true;

  var ROOT = document.getElementById("pmb-root");
  var EX = window.PY_PRACTICE_EX || [];
  if (!ROOT || !EX.length) return;

  var C = {
    green:"#2f6b4f", green2:"#3c8362", greenDk:"#234f3b", greenDeep:"#16352a",
    sage:"#7fa68b", brass:"#a98a4b", ink:"#1a2820", inkSoft:"#41564a", inkMute:"#7a8c80",
    rule:"#cdbfa3", ruleSoft:"#d2dacb", ok:"#2f8f5b", err:"#b3261e"
  };
  ...
  /* ---------- Pyodide (separate, lazy) ---------- */
  var PY_BASE="https://cdn.jsdelivr.net/pyodide/v0.26.4/full/";
  var py=null, pyState="idle", pyPromise=null;
  function setEngine(state){
    pyState=state;
    var pill=document.getElementById("pmb-engine"); if(!pill) return;
    var dot=pill.querySelector(".pmb-led"), txt=pill.querySelector(".pmb-led-txt");
    var map={ idle:[C.brass,"CPython idle"], boot:[C.brass,"loading CPython…"], ready:[C.ok,"CPython ready"],
              nodb:[C.err,"offline — connect to load Python"], err:[C.err,"engine failed to load"] };
    var v=map[state]||map.idle;
  ...
  var store={
    att: ls("commonplace_pypractice_attempts","{}"),
    code: ls("commonplace_pypractice_code","{}"),
    solved: ls("commonplace_pypractice_solved","{}"),
    bank: ls("commonplace_pypractice_bank","{}"),
    saveAtt:function(){ try{ localStorage.setItem("commonplace_pypractice_attempts",JSON.stringify(this.att)); }catch(e){} },
    saveCode:function(){ try{ localStorage.setItem("commonplace_pypractice_code",JSON.stringify(this.code)); }catch(e){} },
    persist:function(){ try{ localStorage.setItem("commonplace_pypractice_solved",JSON.stringify(this.solved)); localStorage.setItem("commonplace_pypractice_bank",JSON.stringify(this.bank)); }catch(e){} },
  ...
  function renderCard(){
    ...
    var host=document.getElementById("pmb-card"); host.innerHTML="";
  ...
  function renderTabs(scroll){
    var host=document.getElementById("pmb-tabs"); computeAllDone();
  ...
  function renderProgress(){
    ...
    document.getElementById("pmb-progress").innerHTML='<div class="pmb-progbar"><div class="pmb-progfill" style="width:'+pct+'%"></div></div><span class="pmb-progtext">'+done+" / "+total+" solved</span>";
  }
  function scrollToTabs(){ var t=document.getElementById("pmb-tabs"); if(t){ window.scrollTo({top:t.getBoundingClientRect().top+window.scrollY-8,behavior:"smooth"}); } }

  /* ---------- Mistake Drill ---------- */
  var drillMode="list", drillQueue=[], drillIdx=0;
  function renderDrill(){
    var host=document.getElementById("pmb-drill"); var banked=store.bankedList();
    var chip=document.getElementById("pmb-drill-count"); if(chip) chip.textContent=banked.filter(function(b){return !b.resolved;}).length+" to drill";
    ...
    host.innerHTML='<div class="pmb-drill-bar"><button type="button" class="pmb-btn pmb-btn-go" id="pmb-drill-start"'+(unresolved.length?"":" disabled")+'>▶ Start drill · '+unresolved.length+'</button><span class="pmb-drill-meta">'+banked.length+" banked · "+(banked.length-unresolved.length)+' resolved</span></div><div class="pmb-bank-list">'+rows+'</div>';
    var sB=document.getElementById("pmb-drill-start"); if(sB) sB.addEventListener("click",function(){ var q=unresolved.map(function(b){return b.key;}); if(!q.length) return; drillQueue=q; drillIdx=0; drillMode="drill"; renderDrill(); });
  ...
  function renderDrillCard(){
    var host=document.getElementById("pmb-drill"); var b=store.bankByKey(drillQueue[drillIdx]);
  ...
  function buildSkeleton(){
    ROOT.innerHTML='<div id="pmb-progress" class="pmb-progress"></div><div id="pmb-tabs" class="pmb-tabs"></div><div id="pmb-card"></div>'
      +'<div class="pmb-drill-section"><div class="pmb-drill-title"><h3>Mistake Drill</h3><span id="pmb-drill-count" class="pmb-drill-chip">0 to drill</span></div><div id="pmb-drill"></div></div>'
  ...
  function boot(){
    injectCss();
    var cnt=document.getElementById("pmb-count"); if(cnt) cnt.textContent=EX.length;
    setEngine("idle"); buildSkeleton();
