...
function pctOf(v) {
  ...
  if (v && typeof v === "object") {
    ...
    /* reading-sync.js shape: { itemKey: "done" | "not_started" } under "crs::<resourceSet>" */
    var ks = Object.keys(v), done = 0, statusy = ks.length > 0;
    for (var i = 0; i < ks.length; i++) {
      var sv = v[ks[i]];
      if (sv !== "done" && sv !== "not_started") { statusy = false; break; }
      if (sv === "done") done++;
    }
    if (statusy) return Math.round(done / ks.length * 100);
  }
  return null;
}
function atOf(v) {
...
function collect() {
  ...
  keys().forEach(function (key) {
    ...
    if (!/^crs::|read|progress|lesson|^py|sync/i.test(key)) return;
