/* learn-shelf.js — replaces the Learn pane of Coding/Python/index.html with the
   three-shelf folder view. Install: copy this file (plus shelf.css, shelf-data.js,
   progress-adapter.js, shelf.html, shelf-page.js) into Coding/Python/, then add
   ONE line before </body> in index.html:

     <script src="learn-shelf.js" defer></script>

   It must load AFTER shelf-data.js and progress-adapter.js:

     <script src="progress-adapter.js"></script>
...
function build() {
  ...
  var mods = S.mods.map(function (m) {
    return fcard({ id: m.id, name: m.t, sub: m.sub, href: "shelf.html?s=" + m.id, written: m.w, read: m.r, total: m.c,
      tag: m.w ? null : "planned",
      count: m.w ? ('<b>' + (m.r ? m.r + ' read &middot; ' : '') + m.w + '</b> / ' + m.c + ' written')
                 : (m.c ? m.c + ' lessons outlined' : 'outline to come') });
  }).join("");
  ...
  pane.innerHTML =
    ...
    + '<div class="shelf-block"><div class="shelf-kick"><b>Books</b> <i>notes from reference reading</i></div><div class="folders">' + books + '</div></div>'
    + '<div class="shelf-block"><div class="shelf-kick"><b>Modules</b> <i>books written from your own notes</i></div><div class="folders">' + mods + '</div></div>'
    + '<div class="shelf-block"><div class="shelf-kick"><b>Notebooks</b> <i>runnable, no reading</i></div><div class="folders">' + nbs + '</div></div>';

  /* the hero's third stat now reports the book, not the page count */
  var hs = document.querySelectorAll(".hero-stats .hstat");
  if (hs.length >= 3) {
    var l = hs[2].querySelector(".hs-l"), v = hs[2].querySelector(".hs-v");
    if (l && v) { l.textContent = "Written"; v.textContent = S.book.written + " of " + S.book.total + " lessons"; }
  }
}
